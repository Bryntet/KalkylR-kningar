...
class Gig:
    def __init__(self, input_RID):
        ...
        self.start_date = self.data.börja_datum.replace(
            hour=0, tzinfo=pytz.timezone("Europe/Stockholm")
        )

        if self.data.sluta_datum is not None:
            self.end_date = self.data.sluta_datum.replace(
                hour=0, tzinfo=pytz.timezone("Europe/Stockholm")
            )
        else:
        ...
        if self.data.prylPaket is not None:
            for paket in self.data.prylPaket:
                ...
                self.output_record.Personal += (
                    paket.personal if paket.personal is not None else 0.0
                )
    ...
    def dagar(self, config, pris):
        assert (
            self.output_record.sluta_datum is not None
            and self.output_record.börja_datum is not None
        )
        ...
        if self.projekt_timmar is None:
            ...
            self.projekt_timmar = math.ceil(
                ...
                * config["projektTid"]
                ...
                if total_personal != 0 and total_personal is not None
                else 0
            )
    ...
    def marginal_rakna(self, config):
        ...
        if (
            self.output_record.Pryl_pris is not None
            and self.output_record.Pryl_pris != 0
        ):
    ...
    def url_make(self):
        ...
        params = {
            ...
            "prefill_Beställare": self.bestallare.id
