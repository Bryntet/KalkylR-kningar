...
class Gig:
    ...
    def rakna_timmar(self, config):
        ...
        if self.data.special_rigg is not None and self.data.special_rigg == 1:
            self.rigg_timmar = (
                self.data.rigg_timmar_spec / self.output_record.Personal
                if self.output_record.Personal is not None
                and self.output_record.Personal > 0.0
                else 0.0
            )
            ...
            self.rigg_timmar = (
                ...
                if self.output_record.Personal is not None
                and self.output_record.Personal > 0
                else 0
            )
    ...
    def post_text_func(self):
        try:
            if self.data.post_text:
                self.post_text_pris = (
                    self.data.Textning_minuter * self.config["textningPostPris"]
                )
                self.post_text_kostnad = (
                    self.data.Textning_minuter * self.config["textningPostKostnad"]
                )
        except KeyError:
            pass
    ...
    def output(self):
        ...
        self.projekt_typ = (
            self.data.Projekt_typ if self.data.Projekt_typ is not None else "live"
        )
    ...
    def url_make(self):
        ...
        params = {
            ...
            "prefill_fldKr9l8iJym15vnv": ",".join(self.adress.id)
