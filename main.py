            ...
            for tid in self.extra_gig_tid:
                ...
                for temp in tid.split("-"):
                    ...
                    date = start_date.replace(
                        day=int(start_date.day), hour=int(temp[0]), minute=int(temp[1])
    ...
    def personal_rakna(self, config):
        ...
        if total_personal != 0:
            ...
            self.odefinerad_personal_kostnad = (
                (total_personal - len(self.person_list)) * self.lön_kostnad * total_tid
            )
            self.odefinerad_personal_pris = (
                (total_personal - len(self.person_list)) * self.timpris * total_tid
            )
        else:
