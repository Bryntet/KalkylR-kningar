...
class Prylob:
    ...
    def dict_make(self):
        ...
        temp_dict.update(
            {"name_packlista": pryl_table.get(self.id)["fields"]["Pryl Namn"]}
        )
...
class Paketob:
    def __init__(self, prylar, saker, config):
        ...
        for pryl in self.prylar:
            self.output_record.Pris += self.prylar[pryl]["pris"] * int(
                self.prylar[pryl]["amount"]
            )
...
class Gig:
    def __init__(self, input_RID):
        ...
        self.person_dict_grouped = {
            ...
            if self.data.__dict__.get(x) is not None
        }
        self.person_list = []
        ...
        self.begin_earlier: float = (
            self.data.Börja_tidigare if self.data.Börja_tidigare is not None else 0
        )
        self.gig_timmar = 0
        self.tim_pris = None
        ...
        self.update = (
            self.data.uppdateraa if self.data.uppdateraa is not None else False
        )
    ...
    def adress_check(self):
        if (
            self.adress.name is not None
            and self.adress.used_time is None
            and not self.svanis
        ):
            ...
            self.adress.time_bike = gmaps_bike["rows"][0]["elements"][0]["duration"][
                "value"
            ]
            if self.adress.time_bike / 60 > 60:
                ...
                self.adress.time_car = gmaps_car["rows"][0]["elements"][0]["duration"][
                    "value"
                ]
            self.adress.distance = (
                self.adress.time_car
                if self.adress.time_car is not None
                else self.adress.time_bike
            )
            self.adress.save()
    ...
    def rakna_timmar(self, config):
        ...
        if self.i_data["specialRigg"]:
            self.rigg_timmar = (
                self.i_data["riggTimmar"] / self.output_record.Personal
                if self.output_record.Personal > 0
                else 0
            )
        else:
            self.rigg_timmar = (
    ...
    def tid(self, config):
        ins = [
            ...
            for combined in (
                self.data.getin_getout
                if self.data.getin_getout is not None
                else "00:00,00:00-00:00,00:00"
            ).split(",")
            for time_thing in (
        ...
        if self.data.tid_för_gig is not None:
            ...
            for starts, ends in zip(self.program_tider[0], self.program_tider[1]):
                self.gig_timmar += (ends.hour + ends.minute / 60) - (
            ...
            for idx, tid in enumerate(self.extra_gig_tid):
                ...
                if self.mgets_outs_true:
                    ...
                    if self.rigg_timmar > 1:  # Om mer än 1 timme gå till följande logik
                        if (
                            self.rigg_timmar > 2
                        ):  # Rigg efter gig max på 1, resten går till innan
                            rigg_after = 1
                ...
                rigg_res_start = datetime.timedelta(
                    ...
                    self.dagar_list.append(
                        (
                            cest.localize(
                                self.start_date
                                + datetime.timedelta(
                                    days=idx, hours=start[0], minutes=start[1]
                                )
                                - rigg_res_start
                            ),
                            cest.localize(
                                self.start_date
                                + datetime.timedelta(
                                    days=idx, hours=end[0], minutes=end[1]
                                )
                                + rigg_res_end
                            ),
                        )
    ...
    def personal_rakna(self, config):
        ...
        if self.projekt_timmar is None:
            ...
            self.projekt_timmar = math.ceil(
                (self.gig_timmar + self.rigg_timmar)
                * config["projektTid"]
                / total_personal
                if total_personal != 0
                else 0
        ...
        total_tid = (
            (self.gig_timmar + self.rigg_timmar + self.projekt_timmar + self.restid * 2)
            * total_personal
            if total_personal > 0
            else 0
        )
        ...
        ) = self.folk.total_cost(
            self.person_list,
            self.tim_dict,
            False,
            self.person_dict_grouped,  # [key for person in self.person_list for key, value in self.person_dict_grouped.items() if person == value]
        )
        ...
        self.levande_video_kostnad = (
            ...
            * (total_personal - self.antal_frilans)
            if total_personal > self.antal_frilans
            else 0
        )

        self.output_record.Personal_kostnad = (
            self.frilans_kostnad + self.levande_video_kostnad
        )
        self.output_record.Personal_pris = (
            self.timpris * total_tid
        )  # Frilans is not used for pris
        ...
        self.output_record.restid = int(
            self.restid
            * float(check_with_default(self.output_record.Personal, 0))
            * 2.0
    ...
    def marginal_rakna(self, config):
        ...
        self.output_record.Pris += (
            self.hyr_pris + self.post_text_pris + self.output_record.Personal_pris
        )
        ...
        try:
            self.marginal = (
                round(
                    self.avkastning
                    / (self.output_record.Pris - self.hyr_things)
                    * 10000
                )
                / 100
            )
    ...
    def output(self):
        ...
        self.output_record.Projekt_timmar = int(
            float(self.gig_timmar)
            * self.output_record.Personal
            * float(len(self.dagar_list))
        )
        self.output_record.Rigg_timmar = int(
            self.rigg_timmar * self.output_record.Personal
        )
            ...
            for idx, (getin, getout) in enumerate(self.dagar_list):
                ...
                if record is not None:
                    ...
                    record.program_stop_hidden = float(
                        ...
                        + self.program_tider[1][idx].minute * 60
                        if self.program_tider[1][idx] is not None
                        else None
                    )
                    record.program_start_hidden = float(
                        self.program_tider[0][idx].hour * 60 * 60
                        + self.program_tider[0][idx].minute * 60
                        if self.program_tider[0][idx] is not None
                        else None
                    )
                    record.actual_getin = (
                    ...
                    record.komma_tillbaka_till_svanis = (
                        getout.hour * 60 * 60 + getout.minute * 60
                    )
    ...
    def url_make(self):
        ...
        params = {
            ...
            "prefill_Frilans": ",".join(
                [x.id for x in self.data.Frilans if x is not None]
            )
            if self.data.Frilans is not None
            else None,
            "prefill_existerande_adress": ",".join(self.adress.id)
            if (type(self.adress) is not str and self.adress is not None)
            else self.adress,
    ...
    def inventarie(self):
        ...
        update = [
            dict_thing
            for dict_thing in inventarie_list
    ...
    def make_tidrapport(self):
        ...
        if self.update:
            ...
            self.old_output[self.output_record.id]["tidrapport"] = tid_out
        ...
        for record in frilans_calcs.all():
            if record["fields"].get("Leveransen") == [self.airtable_record]:
                if record["fields"].get("Riktig kostnad") is not None:
                    ...
                    record["fields"].get("frilans")[0]
                    in self.frilans_personal_dict.keys()
                ):
                    ...
                    self.frilans_personal_dict.pop(
                        record["fields"].get("frilans")[0], None
                    )
                else:
