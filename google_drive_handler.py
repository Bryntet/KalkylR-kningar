...
class Slutkund(Kund):
    def __init__(
        ...
        self.projekt_drid = super().make_if_drid_not_none(
            self.projekt_name, self.slutkund_drid, leverans_drid
        )
...
def main():
    ...
    for projekt in all_projekt:
        ...
        print(
            f"{all_projekt.index(projekt)+1}/{the_len} {all_projekt.index(projekt)/the_len*100}%"
        )
    ...
    if all([x in p_keys for x in ["Kund", "År"]]):
        ...
        if type(year) is not dict:
            ...
            if "Slutkund" in p_keys:
                if (
                    our_clients.get(p_fields["Kund"][0])["fields"]["Kund"].lower()
                    != end_clients.get(p_fields["Slutkund"][0])["fields"][
                        "Name"
                    ].lower()
                ):
                    next_thing = False
                    ...
                    if kund_rid in big_dict_with_all.keys():
                        if year in big_dict_with_all[kund_rid].keys():
                            if slutkund_rid in big_dict_with_all[kund_rid][year].keys():
                                slutkund_drid = big_dict_with_all[kund_rid][year][
                                    slutkund_rid
                                ]["drid"]
                                if (
                                    projekt["id"]
                                    in big_dict_with_all[kund_rid][year][
                                        slutkund_rid
                                    ].keys()
                                ):
                                    projekt_drid = big_dict_with_all[kund_rid][year][
                                        slutkund_rid
                                    ][projekt["id"]]
            ...
            if next_thing:
                x = Kund(
                    service, top_parent, kund_name, kund_rid, year, kund_drid, sublist
                )
                make_proj_rid = True
            url = x.get_link()
            ...
            if make_proj_rid:
                made = x.make_folder(
                    p_fields["Name"].lower(), big_dict_with_all[kund_rid][year]["drid"]
                )
