...
def main():
    ...
    for person_record in people.all():
        if 'E-post' in person_record['fields'].keys():
            emails[person_record['id']
                   ] = person_record['fields']['E-post'].split(', ')
    ...
    lv_emails = [
        epost[0] for epost in emails.values() if '@levandevideo.se' in epost[0]
        or epost[0] == "epost@edvinbryntesson.se"
    ]
    ...
    for event in orm.get_all_in_orm(orm.Projektkalender):
        try:
            ...
            invite_list = [
                person.epost for person in all_people
                if person.epost is not None
            ...
            getin = datetime.datetime.fromisoformat(
                datum.isoformat()
            ...
            if event.program_start is not None and event.program_slut is not None:
                program_start = datetime.datetime.fromisoformat(
                    datum.isoformat()
                ) + datetime.timedelta(seconds=event.program_start)
                program_slut = datetime.datetime.fromisoformat(
                    datum.isoformat()
            ...
            description = ''
            saker = [
                'Projektledare', 'producent', 'Bildproducent', 'Fotograf',
                'Ljudtekniker', 'Ljustekniker', 'Grafikproducent', 'Animatör',
                        ...
                        description += "Producent: \n{}".format(
                            "\n".join([
                                ((namn[x] + " - " + phone_numbers[x])
                                 if x in phone_numbers.keys() else namn[x])
            ...
            if leverans.beställare is not None:
                ...
                if bestallare.phone is not None:
                    description += ' - {}'.format(
                        phone_number(bestallare.phone)
                    )
                description += '\n\n'
            if program_start is not None:
                description += 'Körtider: {}-{}\n'.format(
                    program_start.strftime("%H:%M"),
            ...
            projektkalender.update(
                event.id, {"Calendar description": description}
            )
            ...
            my_event = Event(
                event.name2[2:-2] + (
                    ' [OBEKRÄFTAT]'
                    if event.status == 'Obekräftat projekt' else ""
                ) + (" [RIGG]" if leverans.typ == "Rigg" else ""),
                getin,
                getout,
                location=leverans.Adress[0].name
                if len(leverans.Adress) > 0 else None,
            ...
            if event.id in kalender.keys() and kalender[
                event.id].get("id") is not None:
                if event.id == "m12hrq8qq8ii1at6r5bl6llbt8":
                    print("{my_event}")
                before_update: Event = EventSerializer.to_object(
                    kalender[event.id]
                )
                my_event.event_id = before_update.event_id

                pop_thingies = [
                    "sequence", "iCalUID", "htmlLink", "eventType",
                    "visibility", "etag", "kind", "location", "send_updates"
                ]
                formatted_dict = [
                ...
                for number in (0, 1):
                    for i in range(len(formatted_dict[number]["attendees"])):
                        for popthing in more_thingies:
                            formatted_dict[number]["attendees"][i].pop(
                                popthing, None
                            )
                ...
                ping_conditions = [
                    "attendees", "status", "start", "end", "location"
                ]
                ...
                if formatted_dict[0] == formatted_dict[1]:
                    ...
                    if any(
                        formatted_dict[0].get(dict_key) !=
                        formatted_dict[1].get(dict_key)
